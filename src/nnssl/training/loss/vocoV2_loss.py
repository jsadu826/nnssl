...
class VoCoV2Loss(nn.Module):
    ...
    def __init__(self, pred_weight=1, reg_weight=1, inter_weight=1):
        super(VoCoV2Loss, self).__init__()
    ...
    def prediction_loss(self, base_embeddings_tea, target_embeddings_stu, gt_overlaps) -> float:
        pred_similarity = F.cosine_similarity(
            base_embeddings_tea.detach()[:, None, :, :], target_embeddings_stu[:, :, None, :], dim=-1
        )
    ...
    def inter_volume_loss(
        ...
        pred_similarity = F.cosine_similarity(
            base_embeddings_stu_rolled[:, None, :, :], target_embeddings_stu[:, :, None, :], dim=-1
        )
        logits = F.relu(pred_similarity)

        base_embeddings_tea_rolled = torch.roll(base_embeddings_tea, shifts=1, dims=1)
        gt_similarity = F.cosine_similarity(
            base_embeddings_tea_rolled.detach()[:, None, :, :], target_embeddings_tea.detach()[:, :, None, :], dim=-1
        )
    ...
    def forward(
        ...
        final_loss = self.pred_weight * pred_loss + self.reg_weight * reg_loss + self.inter_weight * inter_loss
        return final_loss
